...
class IGFS:
    ...
    def fit(self, s_number):
        ...
        val_loss_cv = np.zeros(self.t_cv_idx.shape[1])
        # Start cv loop
        for i in range(self.t_cv_idx.shape[1]):
            # Define seed for reproducibility
            check_igfs_opt_utils.set_seed(s_number)
            ...
            X_train_t, y_train_t, X_val_t, y_val_t = check_igfs_opt_utils.torch_tensors(X_train,
                                                                               y_train,
                                                                               X_val,
                                                                               y_val)
            # Load parameters and hyperparameters.
            params = check_igfs_opt_utils.get_params(self.args)
            criterion_rec, criterion_clf, layers, afunc = check_igfs_opt_utils.param_hparam_nn(params, self.args, self.X, self.y)
            epochs = 2500
            # Define NN model and optimizer
            model = check_igfs_opt_utils.autoencoder_E2E(layers, afunc, params)
            ...
            X_train_t, y_train_t, X_val_t, y_val_t, criterion_rec, criterion_clf, model = check_igfs_opt_utils.transfer_device(X_train_t,
                                                                              y_train_t,
                                                                              X_val_t,
                                                                              y_val_t,
                                                                              criterion_rec,
            ...
            best_v_loss, best_t_loss, best_results_t, best_results_v, y_true_total_t, best_y_pred_total_t, y_true_v, best_y_pred_v, best_epochs = check_igfs_opt_utils.train_eval_loop(epochs,
                                                                                                                                                X_train_t,
                                                                                                                                                y_train_t,
                                                                                                                                                X_val_t,
                                                                                                                                                y_val_t,
            ...
            val_loss_cv[i] = best_v_loss
            
        mean_val_loss_cv = np.mean(val_loss_cv)
