...
def train_eval_loop(epochs, train_tensor, y_train_tensor, val_tensor, y_val_tensor, model, optimizer, scheduler, criterion_rec, criterion_clf, params, i, classes):
    best_v_AUC = 0
    best_v_loss = 10000
    epochs_no_improve = 0
    max_epochs_stop = 500
    for epoch in range(epochs):
        ...
        for x_train_batch, y_train_batch in train_batches:
            train_loss, train_acc = train(x_train_batch, y_train_batch, model, optimizer, criterion_rec, criterion_clf, params, weight_rec=params['weight_recon'], weight_bce=params['weight_cl'])
        ...
        _, _, results = clf_results(labels, torch.sigmoid(out), classes)
        AUC = 6
        val_AUC = results[6]
        # Scheduler
        if epoch >= 1000:
            scheduler.step(valid_loss)
        # Save best validation AUC
        if epoch >= 500:
            if val_AUC >= best_v_AUC and valid_loss <= best_v_loss:
                best_v_AUC = val_AUC
