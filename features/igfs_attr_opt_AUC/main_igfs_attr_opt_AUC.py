...
# Import scripts
import load_data
import igfs_attr_opt_AUC, igfs_attr_opt_utils_AUC
...
def main_pipeline(args):
    ...
    X_train = igfs_attr_opt_utils_AUC.data_scaler(X_train)
    ...
    # Run NN and extract IG per validation sample full and correct
        ig_fs = igfs_attr_opt_AUC.IGFS(X_train, y_train_dummy, t_cv_idx, v_cv_idx, args, classes)
        df_attr_acc_class_0, df_attr_acc_class_1, df_attr_full_class_0, df_attr_full_class_1 = ig_fs.fit(seed)
        # Save data
        igfs_attr_opt_AUC.save_data(df_attr_acc_class_0, seed, args, full=False, cl=0)
        igfs_attr_opt_AUC.save_data(df_attr_acc_class_1, seed, args, full=False, cl=1)
        igfs_attr_opt_AUC.save_data(df_attr_full_class_0, seed, args, full=True, cl=0)
        igfs_attr_opt_AUC.save_data(df_attr_full_class_1, seed, args, full=True, cl=1)
