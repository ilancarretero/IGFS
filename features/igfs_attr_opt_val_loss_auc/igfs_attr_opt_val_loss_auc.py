...
import igfs_attr_opt_utils_val_loss_auc
...
class IGFS:
    ...
    def fit(self, s_number):
        ...
        for i in range(self.t_cv_idx.shape[1]):
            ...
            igfs_attr_opt_utils_val_loss_auc.set_seed(s_number)
            ...
            X_train_t, y_train_t, X_val_t, y_val_t = igfs_attr_opt_utils_val_loss_auc.torch_tensors(X_train,
                                                                               y_train,
                                                                               X_val,
                                                                               y_val)
            # Load parameters and hyperparameters.
            params = igfs_attr_opt_utils_val_loss_auc.get_params(self.args)
            criterion_rec, criterion_clf, layers, afunc = igfs_attr_opt_utils_val_loss_auc.param_hparam_nn(params, self.args, self.X, self.y)
            epochs = 2500
            # Define NN model and optimizer
            model = igfs_attr_opt_utils_val_loss_auc.autoencoder_E2E(layers, afunc, params)
            ...
            X_train_t, y_train_t, X_val_t, y_val_t, criterion_rec, criterion_clf, model = igfs_attr_opt_utils_val_loss_auc.transfer_device(X_train_t,
                                                                              y_train_t,
                                                                              X_val_t,
                                                                              y_val_t,
                                                                              criterion_rec,
            ...
            if not os.path.exists(save_model_path):
                ...
                labels, out, epoch = igfs_attr_opt_utils_val_loss_auc.train_eval_loop(epochs,
                                                        X_train_t,
                                                        y_train_t,
                                                        X_val_t,
                                                        y_val_t,
                                                        ...
                                                        save_model_path)
                # Obtain metrics
                y_pred, y_true, results = igfs_attr_opt_utils_val_loss_auc.clf_results(labels, torch.sigmoid(out), self.classes)
                if i == 0:
            ...
            loaded_model = igfs_attr_opt_utils_val_loss_auc.autoencoder_E2E(layers, afunc, params)
            loaded_model.load_state_dict(torch.load(save_model_path))
            submodel = igfs_attr_opt_utils_val_loss_auc.SubModel(loaded_model)
            ...
            sorted_attr_df, correct_idx_preds = igfs_attr_opt_utils_val_loss_auc.IG_attributions(submodel, X_val, y_val, round_pred, val_samples = val_samples)
            ...
            cv_correct_vars_class_1 = igfs_attr_opt_utils_val_loss_auc.selected_correct_vars_attr(self.y, sorted_attr_df, correct_idx_preds, class_1=True, cv=i, full=False)
            cv_correct_vars_class_0 = igfs_attr_opt_utils_val_loss_auc.selected_correct_vars_attr(self.y, sorted_attr_df, correct_idx_preds, class_1=False, cv=i, full=False)
            cv_full_vars_class_1 = igfs_attr_opt_utils_val_loss_auc.selected_correct_vars_attr(self.y, sorted_attr_df, correct_idx_preds, class_1=True, cv=i, full=True)
            cv_full_vars_class_0 = igfs_attr_opt_utils_val_loss_auc.selected_correct_vars_attr(self.y, sorted_attr_df, correct_idx_preds, class_1=False, cv=i, full=True)
