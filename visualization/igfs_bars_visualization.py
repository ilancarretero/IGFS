...
import igfs_bars_visualization_utils
...
def main(args):
    ...
    if args.comparison == 'selector':
        ...
        regex = ['igfs_' + args.type + '_' + args.samples + '_neg',
                 'igfs_' + args.type + '_' + args.samples + '_pos',
                 'igfs_' + args.type + '_' + args.samples + '_pos_neg',
                 'igfs_' + args.type + '_' + args.samples + '_pos_neg_mod']
    elif args.comparison == 'all_rand_igfs':
        regex = ['igfs_median_acc_pos_neg_mod', 'all', 'random']
        
    df_list = []
    for i in range(len(regex)):
        if regex[i] == 'all':
            result_df = igfs_bars_visualization_utils.data_filter_all_vars(df, regex[i])
        else:
            result_df = igfs_bars_visualization_utils.data_filter(df, regex[i], args.vars)
        df_list.append(result_df)
    df_list_mean_std = igfs_bars_visualization_utils.data_wrangling(df_list)
       
    # Visualization
    igfs_names = [x.replace('igfs_', '') for x in regex]
    igfs_names = ['Attr class 0', 'Attr class 1', 'Attr class 0 and 1 (50%)', 'Attr class 0 and 1 (module)']
    igfs_names = ['Mean', 'Median', 'Mode (mean)', 'Mode (median)']
    igfs_bars_visualization_utils.viz_res_igfs(df_list_mean_std, igfs_names, args, viz_path)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--comparison', default='selector', type=str)
    parser.add_argument('--type', default='median', type=str)
